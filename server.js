...
const { getMovies } = require("./1337x.to/movies");
...
app.get("/todaytvseries/:seriesName", (req, res) => {
  ...
  request(
    "http://www.todaytvseries2.com/search-series?searchword=" +
      seriesName +
      "&searchphrase=all",
    (error, response, html) => {
...
app.get("/1337x/:title", (req, res) => {
  ...
  if (!page && !sort) {
    ...
    var url =
      "https://1337x.to/category-search/" + title + "/Movies/" + page + "/";
    getMovies(url, res);
  } else if (!page && sort) {
    var data = {
      title,
...
app.get("/limetorrent/:title", (req, res) => {
  var title = req.params.title;
  var url = " https://www.limetorrents.info/search/movies/" + title + "/";
  getLemons(url,res)
